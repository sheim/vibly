...
s_grid_height = np.linspace(0.5, 1.5, 5)
s_grid_velocity = np.linspace(3, 8, 5)
s_grid = (s_grid_height, s_grid_velocity)
a_grid_aoa = np.linspace(00/180*np.pi, 70/180*np.pi, 71)
# a_grid = (a_grid_aoa, )
a_grid_amp = np.linspace(1.0, 1.75, 4)
...
if True:
    ...
    S_F = vibly.project_Q2S(~Q_F, grids, np.mean)
...
# plt.imshow(S_M, origin='lower')
# plt.show()
>>>>>>> add actuator amplification to daslip
