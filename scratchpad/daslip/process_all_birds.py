...
bird_numbers = ['Bird1', 'Bird2', 'Bird3', 'Bird4', 'Bird5']
# birdNo = 'Bird1'  # 'Bird1','Bird2','Bird3''Bird4''Bird5'
observations = ['ObsH0', 'ObsH4', 'ObsH6']
stepTypes = ['STm3', 'STm2', 'STm1', 'STze', 'STp1', 'STp2', 'STp3']
...
for step_data in it.product(bird_numbers, observations, stepTypes):

    birdNo, observation, stepType = step_data
    print('Selected:'+birdNo+' '+observation+' '+stepType)
    totalTrials = np.shape(dataBlum2014SIUnits['Step'][birdNo][observation]
                           [stepType]['aTD'])[0]
    ...
    if totalTrials == 0:
        continue
    for stepNo in range(totalTrials):
        ...
        if not os.path.exists(basename+'/'+birdNo):
            os.makedirs(basename+'/'+birdNo)
        save_name = basename+'/'+birdNo+'/'+observation+stepType+str(stepNo)
        plt.savefig(save_name + '_fig.pdf', format='pdf')
