...
def com_visualisation(sol, leg_visibility=0.5, colors=colors, size=100, Ground=False):
    ...
    foot_y = result[5, 0]
    plt.plot(
        [foot_x, x_com[0]], [foot_y, y_com[0]], color=colors[0], alpha=leg_visibility
    )
    ...
    if len(t_events[1]) == 0:  # no touch-down
        ...
        if len(t_events[3]) == 0:  # no lift-off
            ...
            if failure:
                fail_index = np.argmax(times > failure)
                plt.plot(
                    x_com[index:fail_index], y_com[index:fail_index], color=colors[1]
                )
                plt.scatter(
                    x_com[fail_index - 1], y_com[fail_index - 1], color="r", s=size
                )
        else:
            liftoff = t_events[3][0]
            lift_index = np.argmax(times > liftoff)
            plt.plot(
                x_com[index - 1 : lift_index],
                y_com[index - 1 : lift_index],
                color=colors[1],
            )
            plt.scatter(
                x_com[lift_index - 1], y_com[lift_index - 1], color=colors[2], s=size
            )
            plt.plot(
                [foot_x, x_com[lift_index - 1]],
                [0, y_com[lift_index - 1]],
                color=colors[2],
                alpha=leg_visibility,
            )
            ...
            if len(t_events[6]) == 0:  # no apex
                ...
                if len(t_events[5]) == 0:  # no fall
                    ...
                    fail_index = np.argmax(times > failure)
                    plt.plot(
                        x_com[lift_index - 1 : fail_index],
                        y_com[lift_index - 1 : fail_index],
                        color=colors[2],
                    )
                    plt.scatter(
                        x_com[fail_index - 1], y_com[fail_index - 1], color="r", s=size
                ...
                plt.plot(
                    x_com[lift_index - 1 : apex_index],
                    y_com[lift_index - 1 : apex_index],
                    color=colors[2],
                )
                plt.scatter(
                    x_com[apex_index - 1],
                ...
                plt.plot(
                    [result[4, apex_index - 1], x_com[apex_index - 1]],
    ...
    if len(t_events[1]) > 0 and len(t_events[3]) > 0 and len(t_events[6]) > 0:
        ...
        if foot:
            ...
            for e in range(3):
                plt.subplot(221)
                plt.plot(
                    times[indices[e] : indices[e + 1]],
                    foot_x[indices[e] : indices[e + 1]],
                    color=colors[e],
                )
                plt.subplot(223)
                plt.plot(
                    times[indices[e] : indices[e + 1]],
                    foot_y[indices[e] : indices[e + 1]],
                    color=colors[e],
                )
                plt.subplot(122)
                plt.plot(
        ...
        for e in range(3):
            for i in range(2):
                for j in range(2):
                    ...
                    plt.plot(
                        times[indices[e] : indices[e + 1] + 1],
                        result[i + 2 * j, indices[e] : indices[e + 1] + 1],
                        color=colors[e],
                    )
            plt.subplot(133)
            plt.plot(
                result[0, indices[e] : indices[e + 1] + 1],
                result[1, indices[e] : indices[e + 1] + 1],
                color=colors[e],
            )
            ## Indicate the events
            for i in range(2):
