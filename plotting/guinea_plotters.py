...
def add_title(p, prepend="", append=""):
    if "damping" in p:
        ...
        title_name = str(
            np.round(p["linear_normalized_damping_coefficient"], decimals=5)
        )
    else:
        title_name = "trial"
...
def plot_ground_perturbations(
    ...
    if draw_LC:
        ...
        if Q_M is not None:
            ...
            ax.plot(
                [x_next[0], foot_x[0]], [x_next[1], foot_y[0]], zorder=2, color=color
            )
            ax.plot(
                [x_next[0], foot_x[-1]], [x_next[1], foot_y[-1]], zorder=2, color=color
            )
            ax.grid = True
...
def waterfall_plot(fig, ax, X, Y, Z, color="viridis", line_width=2):
    """
...
def poincare_plot(
    fig, ax, data, vmax=1, trajectories=None, min_M=0.0, col_offset=0.65, col_norm=1.0
):
    grids = data["grids"]
    extent = [
        grids["states"][1][0],
    ...
    if trajectories is not None:
        ...
        for idx in range(len(trajectories)):
            ...
            if s_m > min_M:
                ...
                ax.scatter(sn[1], sn[0], facecolors="none", edgecolors=col, s=20)
        if idx0 > 0:
            ax.scatter(sn0[1], sn0[0], facecolors="none", edgecolors="black", s=20)
