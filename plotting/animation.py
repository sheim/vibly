...
def animation_visualisation(
    ...
    anim = animation.FuncAnimation(
        fig, animate, init_func=init, frames=duration, interval=interval, blit=True
    )

    if save:
        Writer = animation.writers[writer_name]
        writer = Writer(
            fps=int(1000 / interval), metadata=dict(artist="Me"), bitrate=1800
        )
