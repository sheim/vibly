...
class MeasureLearner:
    ...
    def init_estimation(
        self,
        seed_data,
        prior_model_path="./model/prior.npy",
        ...
        estimation = estimate_measure.MeasureEstimation(
            state_dim=state_dim, action_dim=action_dim, grids=grids, seed=self.seed
        )
        ...
        if learn_hyperparameters:
            ...
            estimation.learn_hyperparameter(
                AS_grid=AS_grid, Q_M=Q_M_proxy, Q_V=Q_V_proxy, save=prior_model_path
            )
    ...
    def sample(
        ...
        Q_V_explore = estimation.safe_level_set(
        ...
        if not thresh_idx.any():  # empty, pick the safest
            ...
            Q_V_prop = estimation.safe_level_set(
                safety_threshold=0, confidence_threshold=None, current_state=s0
            )
        ...
        if failed:
            ...
            if reset is not None:
                ...
                Q_V_full = estimation.safe_level_set(
                    safety_threshold=safety_threshold,
                    confidence_threshold=measure_confidence,
                )
                ...
                if S_M_safe.any():
                    ...
                    s_next_idx = [
                        np.random.choice(safe_idx[i]) for i in range(0, len(safe_idx))
                    ]
                    s_next = [
                        self.grids["states"][i][s_next_idx[i]]
                        for i in range(0, len(s_next_idx))
                    ]
                    ...
                    raise Exception(
                        "The whole measure is 0 now. There exits no action that is safe"
                    )
        ...
        if self.verbose:
            print(
                "State: "
                + np.array2string(s0.reshape(-1), precision=3, separator=", ")
                + " Action: "
                + np.array2string(a.reshape(-1), precision=3, separator=", ")
            )
    ...
    def run(self, n_samples, s0, callback=None, reset_to_s0=False):
        ...
        for ndx in range(n_samples):
            exploration_confidence = self.interpolation(
                self.exploration_confidence_s,
                self.exploration_confidence_e,
                ndx / n_samples,
            )

            measure_confidence = self.interpolation(
                self.measure_confidence_s, self.measure_confidence_e, ndx / n_samples
            )

            safety_threshold = self.interpolation(
                self.safety_threshold_s, self.safety_threshold_e, ndx / n_samples
            )
